...
export default function ResultRow({
  ...
  return (
    <a
      href={url}
      target="_blank"
      className="block relative border min-h-[64px] border-white/10 rounded-lg bg-gradient-to-r from-grey-500/10 to-black-800/10 p-4 my-2 overflow-hidden">
      <div className="flex gap-4">
        ...
        {btc && (
          <div className="flex gap-2">
            <span className="text-xl text-purple-100/80">
              { new Intl.NumberFormat('en-US', {minimumFractionDigits:8}).format(parseFloat(btc))}
            </span>
            <span className="text-xl text-purple-100/50">BTC</span>
          </div>
        )}
      </div>
      {loading && (
        <div className="inset-0 absolute bg-gradient-to-r from-transparent via-black-100/50 to-transparent skeleton-animation border-t border-white/25" />
      )}
    </a>
  );
}